...
class payruedex(Exchange):
    ...
    def fetch_markets(self, params={}):
        ...
        markets = self.publicGetInfo(request)
        currenciesById = {}
    ...
    def fetch_ohlcv(self, symbol, timeframe='1h', since=None, limit=None, params={}):
        ...
        response = self.publicGetOhlcv(self.extend(parameters, params))
        return response['ohlcv']
        # response = self.publicGetInfo(self.extend(parameters, params))
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        response = self.publicGetInfo(self.extend(request, params))
        ticker = None
        ids = symbol.split('/')
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        if type == 'limit':
            ...
            data = self.getPayRueDexOrderHash(requestToHash)
            print(data)
            ...
            response = self.privatePostSubmitOrder(request)  # self.extend(request, params) will cause invalid signature
            ...
            estimatedAmountResponse = self.publicGetEstimateMarketOrder(estimatedPriceRequest)
            ...
            response = self.privatePostSubmitOrder(request)
            return self.parse_order(response['order'], market)
    ...
    def cancel_order(self, orderId, symbol=None, params={}):
        ...
        response = self.privatePostCancelOrder(self.extend(request))
        # {success: 1}
    ...
    def deposit(self, code, amount, txHash, params={}):
        self.check_required_dependencies()
        ...
        response = self.privatePostDeposit(self.extend(request, params))
        return {
            'info': response,
            'id': None,
        }
